...
s = raw_input("Liste des taux de migrations voulus, separes par des espaces: ")
migra=map(float, s.split())
migra_list=list()
for i in migra:
...
with PdfPages("graphe_simu_stop_Several_Migrationrate_{}_generations_{}_repeats_{}_mu_{}_ind_{}_h_{}_s.pdf".format(param["nbGeneration"], param["nbRepeats"], param["txMut"], param["popSize"], param["h"], param["fitness"])) as pdf:
	for i in range((len(seqLength)+1)):
...
np.savetxt("out_simu_stop_{}_generations_{}_repeats_{}_mu_{}_ind_{}_h_{}_s.csv".format(param["nbGeneration"], param["nbRepeats"], param["txMut"], param["popSize"], param["h"], param["fitness"]), countMutaMatrix, delimiter=",")

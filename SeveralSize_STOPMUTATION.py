...
parser.add_argument('-N', '--Ne', type=float, nargs='+', required=True, help="Effective population sizes separated by spaces")		#Effective population size (estimated to be between 100 and 500 (Fumey and Casane 2017, BioxRiv))
...
outputname=','.join(str(e) for e in Ne_list)
...
for Ne in Ne_list:
	...
	np.savetxt("out_simu_stop_{}_generations_{}_repeats_{}_mu_{}_migra_{}_migrants_{}_mig_{}_h_{}_s_{}_popsizes_{}_MatriceIndice.csv".format(param["nbGeneration"], param["nbRepeats"], param["txMut"], param["probMigra"], param["indMigra"],param["txmigra"], param["h"], param["fitness"], outputname, Ne), countMutaMatrix, delimiter=",")
...
with PdfPages("graphe_simu_stop_Several_Ne_{}_generations_{}_repeats_{}_mu_{}_migra_{}_migrants_{}_mig_{}_h_{}_s_{}_Popsizes.pdf".format(param["nbGeneration"], param["nbRepeats"], param["txMut"],param["probMigra"], param["indMigra"],param["txmigra"], param["h"], param["fitness"], outputname)) as pdf:
	for i in range((len(seqLength)+1)):
		for N in range((len(countMutaMatrixF))):
